						...
						name="NoticeModify" enctype="multipart/form-data" method="POST"	id="form">
							<div class="section2">
								...
								<hr>
								   ...
								   <tr>
		                              <td id="fileIndex" colspan='2'><c:forEach var="NoticeFile"
		                                    items="${NoticeFile}" varStatus="var">
		                                    <div>
		                                       <input type="hidden" id="bFileID"
		                                          name="BFileID${var.index}"
		                                          value="${NoticeFile.BFileID}"> <input
		                                          type="hidden" id="originalFileName" name="FILE_NAME"
		                                          value="OriginalFileName${var.index}"> <a href="#"
		                                          id="fileName"
		                                          onclick="FileDown('${NoticeFile.BFileID}'); return false;">${NoticeFile.BOriginalFileName}</a>(${NoticeFile.BFileSize}kb)
		                                       <input type="button" id="fileDel"
		                                          onclick="fnDel'${NoticeFile.BFileID}','BFileID${var.index}');"
		                                          value="삭제"> <br>
		                                    </div>
		                                 </c:forEach></td>
		                           </tr>
							...
							<div id="btn">
								...
								<input type="submit" value="수정 완료" id="listButton"> <a href="${path}/noticeList">
