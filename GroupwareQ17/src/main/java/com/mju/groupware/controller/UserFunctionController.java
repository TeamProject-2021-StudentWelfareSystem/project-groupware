...
public class UserFunctionController {
	...
	public String home(User user, Principal principal, Model model, HttpServletRequest request, Student student) {
		...
		AddBoardListModel(model, this.ConstantHome.getNL(), NoticeList);
		// 커뮤니티 리스트 띄우기
		List<Board> CommunityList = boardService.SelectCommunityBoardList();
		AddBoardListModel(model, this.ConstantHome.getCL(), CommunityList);
	...
	public String BlankHome(User user, Principal principal, Model model, HttpServletRequest request, Student student) {
		...
		// 공지사항 리스트 띄우기
		List<Board> NoticeList = boardService.SelectNoticeBoardList();
		AddBoardListModel(model, "noticeList", NoticeList);
		// 커뮤니티 리스트 띄우기
		List<Board> CommunityList = boardService.SelectCommunityBoardList();
		AddBoardListModel(model, this.ConstantHome.getCL(), CommunityList);
	...
	// 마이페이지 - 내 게시글 조회
		...
		public String myPostList(Model model, Principal principal, User user, Criteria cri) {
			// 유저 정보
			...
			pageMaker.setTotalCount(boardService.CountTotalMyPostList());
			
			List<Board> list = boardService.SelectMyBoardList(UserID);
			model.addAttribute("MyBoardList", list);
//			List<Board> list = boardService.SelectMyPostListPN(cri);
//			model.addAttribute("MyBoardList", list);
//			model.addAttribute("pageMaker", pageMaker);
	...
	public String myInquiryList(Model model, Principal principal, User user, Criteria cri) {
		// 유저 정보
		...
		PageMaker pageMaker = new PageMaker();
		pageMaker.setCri(cri);
		pageMaker.setTotalCount(inquiryService.CountTotalMyInquiryList());
		
		//List<Inquiry> list = inquiryService.SelectMyInquiryListPN(cri, UserID);
		List<Inquiry> list = inquiryService.SelectMyInquiryList(UserID);
		if (!list.isEmpty()) {
			model.addAttribute(this.ConstantMyInquiryList.getMIList(), list);
			model.addAttribute("pageMaker", pageMaker);
		}
...
//		}
	...
	public String DoEmail(User user, UserEmail userEmail, RedirectAttributes redirectAttributes, Model model,
		...
		if ((String) request.getParameter(constantDoEmail.getEM()) != null) {
			AddStringModel(model, constantDoEmail.getEM(), UserEmail);
		...
		if ((String) request.getParameter(constantDoEmail.getAuthNum()) != null) {
			AddStringModel(model, constantDoEmail.getAuthNum(),
	...
	public String DoSignUp(User user, Student student, RedirectAttributes redirectAttributes, Model model,
		...
		if ((String) request.getParameter("UserLoginID") != null)
			AddStringModel(model, "UserLoginID", UserLoginID);
		if ((String) request.getParameter(constantDoSignUp.getPwd()) != null)
			AddStringModel(model, constantDoSignUp.getPwd(), (String) request.getParameter(constantDoSignUp.getPwd()));
		if ((String) request.getParameter(constantDoSignUp.getSName()) != null)
			AddStringModel(model, constantDoSignUp.getSName(),
					(String) request.getParameter(constantDoSignUp.getSName()));
		if ((String) request.getParameter("StudentGender") != null)
			AddStringModel(model, "StudentGender", StudentGender);
		if ((String) request.getParameter("UserPhoneNum") != null)
			AddStringModel(model, constantDoSignUp.getPhoneNum(),
					(String) request.getParameter(constantDoSignUp.getPhoneNum()));
		if ((String) request.getParameter(constantDoSignUp.getSNum()) != null)
			AddStringModel(model, constantDoSignUp.getSNum(), request.getParameter(constantDoSignUp.getSNum()));
		if ((String) request.getParameter("StudentGrade") != null)
			AddStringModel(model, "StudentGrade", StudentGradeForSignUp);
		if ((String) request.getParameter("UserColleges") != null)
			AddStringModel(model, "UserColleges", StudentColleges);
		if ((String) request.getParameter("UserMajor") != null)
			AddStringModel(model, "UserMajor", StudentMajor);
		if ((String) request.getParameter("StudentDoubleMajor") != null)
			AddStringModel(model, "StudentDoubleMajor", StudentDoubleMajor);
		...
		if (request.getParameter("IdCheck") != null) {
			...
			if (UserLoginID.equals("")) {
				...
				if (Checker) {
					UserLoginID = "";
					AddStringModel(model, "check", UserLoginID);
					Checker = false;
					response.setContentType("text/html; charset=UTF-8");
					PrintWriter Out = response.getWriter();
	...
	public String findPassword(User user, RedirectAttributes redirectAttributes, Model model,
		...
		if (request.getParameter("IdCheck") != null) {
			...
			if (IDChecker) {
				AddStringModel(model, "UserLoginID", UserLoginID);
				AddStringModel(model, constantDoFindPassword.getUName(),
			...
			if (UserEmail.equals("")) {
				...
				AddStringModel(model, constantDoFindPassword.getUName(),
						(String) request.getParameter(constantDoFindPassword.getUName()));
				AddStringModel(model, "UserEmail", UserEmail);
			...
			if (NameChecker) {
				AddStringModel(model, constantDoFindPassword.getAuthNum(),
			...
			user.setUserLoginPwd(HashedPw);
			AddStringModel(model, "UserLoginPwd", NewPwd);
	...
	public String emailList(HttpServletRequest request, Model model, Principal principal, User user) {
		...
		if (EmailList.isEmpty()) {
			...
			AddEmailListModel(model, "EmailList", EmailList);
			return constantDoEmail.getEURL();
		}
	}
	...
	public String emailContent(HttpServletRequest request, Model model, Principal principal, User user) {
		...
		if (EmailList.isEmpty()) {
			...
			AddStringModel(model, "EmailSender", EmailList.get(IntNum).getFrom());
			AddStringModel(model, "EmailTitle", EmailList.get(IntNum).getTitle());
			AddStringModel(model, "EmailDate", EmailList.get(IntNum).getDate());
			AddStringModel(model, "EmailContent", EmailList.get(IntNum).getContent());
		}
		return constantDoEmail.getECURL();
	}
	
	private void AddStringModel(Model model, String ConstantName, String ParameterName) {
		model.addAttribute(ConstantName, ParameterName);
	}
	private void AddBoardListModel(Model model, String ConstantName, List<Board> ListName) {
		model.addAttribute(this.ConstantHome.getNL(), ListName);
	}
	private void AddEmailListModel(Model model, String ConstantName, List<UserEmail> ListName) {
		model.addAttribute(ConstantName, ListName);
	}
	private void PrintScript(HttpServletResponse response) throws IOException {
		PrintWriter Out = response.getWriter();
