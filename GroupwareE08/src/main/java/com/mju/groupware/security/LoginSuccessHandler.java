...
public class LoginSuccessHandler implements AuthenticationSuccessHandler { // 로그인 성공 후 부가작업
	// https://to-dy.tistory.com/92?category=720806 <- 로그인 '실패' 후 부가작업
	...
	private void clearAuthenticationAttributes(HttpServletRequest request) {
		...
		session.removeAttribute(WebAttributes.AUTHENTICATION_EXCEPTION); // WebAttributes.AUTHENTICATION_EXCEPTION 이름
																			// 값으로 정의된 세션을 지움
	}
	...
	protected void resultRedirectStrategy(HttpServletRequest request, HttpServletResponse response,
		...
		if (savedRequest != null) { // 세션에 이동할 url의 정보가 담겨져 있을 때(인증 권한이 필요한 페이지에 접근했을 경우)
			String TargetUrl = savedRequest.getRedirectUrl(); // savedRequest 객체를 통해 getRedirectUrl (로그인화면을 보기 전에 갔던
																// url)을 가져옴
