                              ...
                              <div class="col-xs-4 mr-4">
                                 <form class="d-none d-sm-inline-block form-inline mr-auto ml-md-3 my-2 my-md-0 mw-100 navbar-search mt-3 pt-3" >
                                          <div class="input-group">
                                             ...
                                             aria-label="Search" aria-describedby="basic-addon2" style="width:72px;">
                                                    <option value="l">강의실 예약/이용</option>
                                                    <option value="t">팀원관리</option>
                              ...
                              <form action="" name="InquiryList" method="POST" id="form">
                                 ...
                                 <table class="table table-bordered dataTable" id="dataTable"
                                    ...
                                    <thead>
                                       <tr role="row">
                                          ...
                                          <th class="sorting sorting_asc" tabindex="0"
                                             ...
                                             style="width: 7%;">작성자</th>
                                          <th class="sorting sorting_asc" tabindex="0"
                                             aria-controls="dataTable" rowspan="1" colspan="1"
                                             aria-sort="ascending"
                                             aria-label="Name: activate to sort column descending"
                                             style="width: 10%;">문의 분류</th>
                                          <th class="sorting" tabindex="0" aria-controls="dataTable"
                                             rowspan="1" colspan="1"
                                             aria-label="Start date: activate to sort column ascending"
                                             style="width: 13%;">작성일</th>
                                          <th class="sorting" tabindex="0" aria-controls="dataTable"
                                             rowspan="1" colspan="1"
                                             aria-label="Age: activate to sort column ascending"
                                             style="width: 10%;">답변 상태</th>
                                    ...
                                    <c:set var="UserID" value="${UserID}" />
                                       ...
                                       <c:forEach items="${inquiryList}" var="inquiryList" varStatus="status">
                                       <tr class="odd">
                                          <td><c:out value="${pageMaker.totalCount - ((pageMaker.cri.page-1) * 10) - status.index}"/></td>
                                          <td id="title" title="${inquiryList.getIBoardSubject()}">
                                             <a href="${path}/inquiryContent?no=${inquiryList.getIBoardID()}">
                                             <c:out value="${inquiryList.getIBoardSubject()}" /></a>
                                          </td>
                                          <td><c:out value="${inquiryList.getIBoardWriter()}" /></td>
                                          <c:if test="${inquiryList.getIBoardType()=='lectureRoom'}">
                                             <td>강의실 이용/예약</td>
                                          </c:if>
                                          <c:if test="${inquiryList.getIBoardType()=='teamMembers'}">
                                             <td>팀원 관리</td>
                                          </c:if>
                                          <c:if test="${inquiryList.getIBoardType()=='community'}">
                                             <td>커뮤니티</td>
                                          </c:if>
                                          <c:if test="${inquiryList.getIBoardType()=='review'}">
                                             <td>후기</td>
                                          </c:if>
                                          <c:if test="${inquiryList.getIBoardType()=='suggestion'}">
                                             <td>건의</td>
                                          </c:if>
                                    ...
                                    <!-- 페이징처리 시작 -->
                                     <ul class="pagination">
                                         <li class="paginate_button page-item previous ${(pageMaker.cri.page==1) ||(pageMaker.cri.page)==(pageMaker.endPage) ? 'disabled':''}"
                                                   ...
                                                   <a href='<c:url value="/inquiryList?page=${pageMaker.startPage-1}"/>'
                                                   aria-controls="dataTable" data-dt-idx="0" tabindex="0"
                                                   class="page-link">Previous</a></li>
                                                
                                                <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="pageNum">
                                                <li class="paginate_button page-item ${pageMaker.cri.page==pageNum ? 'active':'' }">
                                                   <a href='<c:url value="/inquiryList?page=${pageNum}"/>' 
                                               ...
                                               <c:choose>            
                                                ...
                                                <li class="paginate_button page-item next" id="dataTable_next">
                                                   <a href='<c:url value="/inquiriyList?page=${pageMaker.endPage+1 }"/>'
