...
public class UserFunctionController {
	...
	public String detail(HttpServletRequest request, Model model) throws IOException {
		...
		/*if (MysqlRole == "STUDENT") {
			return "redirect:myPageStudent";
		} else if (MysqlRole == "PROFESSOR") {
			return "redirect:myPageProfessor";
		} else if (MysqlRole == "ADMINISTRATOR") {
			return "redirect:mjuAdminLogin";
		}
		return "/";*/
	}
	...
	public String DoWithdrawl(HttpServletRequest request, Principal Principal, User user, Student student,
		...
		if ((String) request.getParameter(ConstantWithdrawal.getParameter1()) != null) {
			...
			if (UserInfo.getUserRole().equals(ConstantWithdrawal.getParameter2())) {
				...
				// 탈퇴한 날짜 set
				Date Now = new Date();
				SimpleDateFormat Date = new SimpleDateFormat(ConstantWithdrawal.getParameter4());
			...
			// 교수가 탈퇴하면
			if (UserInfo.getUserRole().equals(ConstantWithdrawal.getParameter3())) {
				user.setUserRole(ConstantWithdrawal.getParameter3());
				// 탈퇴한 날짜 set
				Date Now = new Date();
				SimpleDateFormat Date = new SimpleDateFormat(ConstantWithdrawal.getParameter4());
				user.setWithdrawalDate(Date.format(Now));
				...
				professor.setProfessorRoom(ProfessorInfo.getProfessorRoom());
				professor.setProfessorRoomNum(ProfessorInfo.getProfessorRoomNum());
