        ...
        <div id="content-wrapper" class="d-flex flex-column">
            <div id="content">
				...
				<div class="card-header py-3">
					<h5 class="m-0 font-weight-bold text-primary">팀 자료 공유</h5>
				</div>
				<div class="card-body">
               	
				<form action="DocumentDelete.do?tBoardID=${TBoardID}&&teamID=${TeamID}"
							name="DocumentDelete" method="POST" id="form">
					<div class="form-group col-sm-12">
						<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />
						
						<input type="text" class="input-sm input-tp font-weight-bold" id="boardSubject" name="BoardSubject" disabled readonly
							value="${BoardSubject}">
						<div class="form-group text-gray-800 row ml-1 my-1">
							<label for="" class="col-form-label mr-2"><span class="small">작성자</span></label>
							<input type="text" class="input-sm input-tp small" name="BoardWriter" id="boardWriter" 
						...
						<div class="form-group row my-3">
							<div class="card-body">
								<div class="ckeditor_contents">
									<p class="card-text"><c:out value="${BoardContent}" escapeXml="false"/></p>
								</div>
							</div>
						</div>
						<hr>
				         <div class="form-group row">
				         	...
				         	<div class="col-sm-10 my-2" id="fileIndex">
								<c:forEach var="TeamBoardFile" items="${TeamBoardFile}">
					         	<a href="#" onclick="TeamBoardFileDown('${TeamBoardFile.TFileID}'); return false;">${TeamBoardFile.TOriginalFileName}</a>(${TeamBoardFile.TFileSize}kb)<br>
					...
					<div class="user text-center row p-3 ml-5 justify-content-center">
			            <a href="${path}/team/documentList?no=${TeamID}"><input type="button" value="목록" id="listButton" class="btn btn-secondary btn-user mr-2"></a>
							<c:set var="TUserID" value="${TUserID}" />
							<c:set var="TUserIDFromWriter" value="${TUserIDFromWriter}" />
							<c:if test="${TUserID == TUserIDFromWriter}">
								<a href="${path}/team/documentModify?tBoardID=${TBoardID}&&TeamID=${TeamID}">
