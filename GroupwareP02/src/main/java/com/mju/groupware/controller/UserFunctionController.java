...
public class UserFunctionController {
	...
	public String modifyPassword(HttpServletResponse response, HttpServletRequest request, User user,
		...
		String HashedPwd = BCrypt.hashpw(NewUserLoginPwd, BCrypt.gensalt());// 바꿀 비밀번호 암호화
		user.setUserModifiedPW(HashedPwd);
		String NewUserLoginPwdCheck = (String) request.getParameter("UserNewPwdCheck");

		// 새로입력한 비밀번호와 확인 비밀번호가 일치하면
		if (NewUserLoginPwd.equals(NewUserLoginPwdCheck)) {
