					...
					<div class="card shadow mb-4">
						<div class="card-text p-3">
							<ul class="nav nav-tabs">
							  ...
							  <li class="nav-item">
							    <a class="nav-link active border-bottom-primary"  aria-current="page" href="${path}/admin/manageSecession">탈퇴 계정 관리</a>
							  </li>
							</ul>
						</div>
						<div class="card-body">
								...
								<div id="dataTable_wrapper"
                    						...
                    						<script>
												...
												function setSearchTypeSelect(){
													...
													$('#searchBtn').on('click',function(){
														...
														var url = "${path}/admin/manageSecession?page=1"
									...
									<form action="${path}/admin/manageList.do" name="ManageList"
										...
										<table class="table table-bordered dataTable sleepUserList"
											...
											<c:forEach items="${SelectWithdrawalUserList}"
												var="SelectWithdrawalUserList" varStatus="status">
												<tr>
													<td class="col-md-1"><input type="checkbox"
														name="RowCheck" class="check"
														value="${SelectWithdrawalUserList.getUserLoginID()}" /></td>
													<td><c:out value="${pageMaker.totalCount - ((pageMaker.cri.page-1) * 10) - status.index}" /></td>
													<td><c:out
															value="${SelectWithdrawalUserList.getUserLoginID()}" /></td>
													<td><c:out
															value="${SelectWithdrawalUserList.getUserName()}" /></td>
													<td><c:out
															value="${SelectWithdrawalUserList.getUserPhoneNum()}" /></td>
													<td><c:out
															value="${SelectWithdrawalUserList.getUserEmail()}" /></td>
													<c:if test="${SelectWithdrawalUserList.getUserRole()=='STUDENT'}">
														<td>학생</td>
													</c:if>
													<c:if test="${SelectWithdrawalUserList.getUserRole()=='ADMIN'}">
														<td>관리자</td>
													</c:if>
													<c:if test="${SelectWithdrawalUserList.getAuthority()=='ROLE_USER'}">
														<td>사용자</td>
													</c:if>
													<c:if test="${SelectWithdrawalUserList.getAuthority()=='ROLE_ADMIN'}">
														<td>관리자</td>
													</c:if>
													<td class="col-md-2"><c:out value="${SelectWithdrawalUserList.getLoginDate()}" /></td>
												</tr>
											</c:forEach>
										</table>
												...
												<!-- 페이징처리 시작 -->
												    <ul class="pagination">
												        <li class="paginate_button page-item previous ${(pageMaker.cri.page==1) ? 'disabled':''}"
				                                          ...
				                                          <a href='<c:url value="/admin/manageSecession${pageMaker.makeQuery(pageMaker.startPage-1)}"/>'
				                                          aria-controls="dataTable" data-dt-idx="0" tabindex="0"
				                                          class="page-link">Previous</a></li>
				                                       
				                                       <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="pageNum">
				                                       <li class="paginate_button page-item ${pageMaker.cri.page==pageNum ? 'active':'' }">
				                                          <a href='<c:url value="/admin/manageSecession${pageMaker.makeQuery(pageNum)}"/>' 
				                                       ...
				                                       <li class="paginate_button page-item next" id="dataTable_next">
				                                          <a href='<c:url value="/admin/manageSecession${pageMaker.makeQuery(pageMaker.endPage + 1)}"/>'
