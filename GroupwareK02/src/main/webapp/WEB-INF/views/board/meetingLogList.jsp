			...
			<div class="rightBox">
				...
				<section>
					<div class="section2">
						...
						<form action="" name="MeetingLogList" method="POST" id="form">
							<input type="hidden" name="${_csrf.parameterName}"
								value="${_csrf.token}" />
							<table class="meetingLogList">
								<thead>
									<tr>
										<th class="col1">번호</th>
										<th class="col2">제목</th>
								...
								<tbody>
									<c:forEach items="${meetingLogList}" var="meetingLogList"
										varStatus="status">
										<tr>
											<td><c:out value="${meetingLogList.getBoardID()}" /></td>
											<td><a href="${path}/meetingLogContent?no=${meetingLogList.getBoardID()}">
												<c:out value="${meetingLogList.getBoardSubject()}" /></a></td>
											<td><c:out value="${meetingLogList.getBoardWriter()}" /></td>
											<td><c:out value="${meetingLogList.getBoardDate()}" /></td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
							...
							<div id="page" class="btn">
								<input type="button" value="←" id="leftList"> <input
									...
									href="${path}/team/meetingLogWrite"><input type="button"
