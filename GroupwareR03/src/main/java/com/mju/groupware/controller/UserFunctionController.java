...
public class UserFunctionController {
	...
	public String myPostList(Model model, Criteria cri,Principal principal, User user) {
		// 유저 정보
		this.ConstantMyPostList = (ConstantMyPostList) ctx.getBean("MyPostList");
		GetUserInformation(principal, user, model);
		int UserID = Integer.parseInt(boardService.SelectLoginUserID(principal.getName()));
		
		String UserLoginID = principal.getName();
		PageMaker pageMaker = new PageMaker();
		pageMaker.setCri(cri);
		cri.setUserID(UserID);
		pageMaker.setTotalCount(boardService.CountTotalMyPostList(cri));
		
		cri.setUserLoginID(UserLoginID);
		List<Board> postList = boardService.SelectMyPostListPN(cri);
		
		model.addAttribute("MyBoardList", postList);
	...
	public String myInquiryList(Model model, Principal principal, User user, Criteria cri) {
		...
		cri.setUserID(UserID);
		pageMaker.setCri(cri);
		pageMaker.setTotalCount(inquiryService.CountTotalMyInquiryList(cri));
		
		cri.setUserLoginID(UserLoginID);
		
		List<Inquiry> MyInquiryList = inquiryService.SelectMyInquiryListPN(cri);
